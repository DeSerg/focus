...
def se_module(input_net, ratio=16, reuse = None, scope = None):
    with tf.compat.v1.variable_scope(scope, 'SE', [input_net], reuse=reuse):
        h,w,c = tuple([dim for dim in input_net.shape[1:4]])
        assert c % ratio == 0
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        with tf.compat.v1.variable_scope('SphereNet', [images], reuse=reuse):
            ...
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=False):
                ...
                print('SphereNet input shape:', [dim for dim in images.shape])
